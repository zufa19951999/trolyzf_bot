...
try:
    ...
    def optimize_database():
        try:
            ...
            c = conn.cursor()
            c.execute("VACUUM")
            c.execute('''DELETE FROM alerts WHERE triggered_at IS NOT NULL 
        ...
        try:
            ...
            c.execute('''CREATE TABLE IF NOT EXISTS group_admins
                         (id INTEGER PRIMARY KEY AUTOINCREMENT,
                          ...
                          created_at TEXT, UNIQUE(group_id, admin_id))''')

            c.execute('''CREATE TABLE IF NOT EXISTS permission_logs
                         (id INTEGER PRIMARY KEY AUTOINCREMENT,
                          group_id INTEGER, action_by INTEGER, target_user INTEGER,
                          action TEXT, old_role TEXT, new_role TEXT, created_at TEXT)''')
    ...
    # ==================== EXPENSE FUNCTIONS ====================
    def add_expense_category(owner_id, name, budget=0):
        conn = None
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            ...
            c.execute('''INSERT INTO expense_categories 
                         ...
                         VALUES (?, ?, ?, ?)''',
                      (owner_id, name.upper(), budget, created_at))
            conn.commit()
            return True
        except Exception as e:
    ...
    def get_recent_incomes(owner_id, limit=10):
        conn = None
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
    ...
    def get_income_by_period(owner_id, period='month'):
        conn = None
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
    ...
    def get_expenses_by_period(owner_id, period='month'):
        conn = None
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
    ...
    def get_balance_summary(owner_id, period='month'):
        try:
            if period == 'day':
                incomes = get_income_by_period(owner_id, 'day')
                expenses = get_expenses_by_period(owner_id, 'day')
                title = f"HÔM NAY ({get_vn_time().strftime('%d/%m/%Y')})"
            elif period == 'month':
                incomes = get_income_by_period(owner_id, 'month')
                expenses = get_expenses_by_period(owner_id, 'month')
                title = f"THÁNG {get_vn_time().strftime('%m/%Y')}"
            elif period == 'year':
                incomes = get_income_by_period(owner_id, 'year')
                expenses = get_expenses_by_period(owner_id, 'year')
                title = f"NĂM {get_vn_time().strftime('%Y')}"
            else:
                conn = sqlite3.connect(DB_PATH)
                c = conn.cursor()
                c.execute('''SELECT currency, SUM(amount) FROM incomes 
                             WHERE user_id = ? GROUP BY currency''', (owner_id,))
    ...
    def delete_expense(expense_id, owner_id):
        ...
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute('''DELETE FROM expenses WHERE id = ? AND user_id = ?''', (expense_id, owner_id))
            conn.commit()
            return c.rowcount > 0
        except Exception as e:
    ...
    def delete_income(income_id, owner_id):
        conn = None
        try:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute('''DELETE FROM incomes WHERE id = ? AND user_id = ?''', (income_id, owner_id))
            conn.commit()
            return c.rowcount > 0
        except Exception as e:
    ...
    def delete_category(category_id, owner_id):
        ...
        try:
            ...
            c.execute('''DELETE FROM expenses WHERE category_id = ? AND user_id = ?''', 
                     (category_id, owner_id))
            deleted_expenses = c.rowcount
            c.execute('''DELETE FROM expense_categories WHERE id = ? AND user_id = ?''', 
                     (category_id, owner_id))
            conn.commit()
        ...
        if len(ctx.args) == 1:
            ...
            try:
                ...
                c.execute('''UPDATE portfolio SET amount = ?, buy_price = ?, total_cost = ?
                          ...
                          (new_amount, new_price, new_total, tx_id, owner_id))
                conn.commit()
                affected = c.rowcount
                conn.close()
    ...
    async def alerts_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        owner_id = ctx.bot_data.get('effective_user_id', update.effective_user.id)
        alerts = get_user_alerts(owner_id)
    ...
    async def export_csv_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if query.data == "export_csv":
                ...
                timestamp = get_vn_time().strftime('%Y%m%d_%H%M%S')
                filename = f"portfolio_{owner_id}_{timestamp}.csv"
                ...
                await query.edit_message_text(
                    f"💰 *MENU ĐẦU TƯ COIN*\n━━━━━━━━━━━━━━━━\n\n🕐 {format_vn_time()}",
                    parse_mode=ParseMode.MARKDOWN,
                    reply_markup=get_invest_menu_keyboard(owner_id, query.message.chat.id)
                )
                
            elif query.data == "expense_export":
                expenses = get_recent_expenses(owner_id, 1000)
                incomes = get_recent_incomes(owner_id, 1000)
                ...
                timestamp = get_vn_time().strftime('%Y%m%d_%H%M%S')
                filename = f"expense_report_{owner_id}_{timestamp}.csv"
                filepath = os.path.join(EXPORT_DIR, filename)
    ...
    async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            if data == "back_to_main":
                ...
                if current_user_id != target_user_id and not check_permission(chat_id, current_user_id, 'view'):
                    await query.edit_message_text(
                ...
                uid = ctx.bot_data.get('effective_user_id', query.from_user.id)
                ...
                for cat in categories:
                    ...
                    if current_user == owner_id or check_admin_permission(query.message.chat.id, current_user, 'delete'):
                ...
                await query.edit_message_text(
                    balance_msg, parse_mode=ParseMode.MARKDOWN,
                    reply_markup=InlineKeyboardMarkup(keyboard)
                )
