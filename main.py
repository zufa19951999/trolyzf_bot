...
Author: Assistant
Version: 3.0 - Structured for Easy Maintenance
"""
...
class RenderConfig:
    ...
    def get_worker_count(self):
        return max(1, int(self.cpu_limit) * 2) if self.is_render else 4
    ...
    try:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS portfolio 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, symbol TEXT, 
                     amount REAL, buy_price REAL, buy_date TEXT, total_cost REAL)''')
        
        c.execute('''CREATE TABLE IF NOT EXISTS alerts 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, symbol TEXT, 
                     target_price REAL, condition TEXT, is_active INTEGER DEFAULT 1, 
                     created_at TEXT, triggered_at TEXT)''')
        
        c.execute('''CREATE TABLE IF NOT EXISTS expense_categories 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, name TEXT, 
                     budget REAL, created_at TEXT)''')
        
        c.execute('''CREATE TABLE IF NOT EXISTS expenses 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, category_id INTEGER, 
                     amount REAL, currency TEXT DEFAULT 'VND', note TEXT, expense_date TEXT, 
                     created_at TEXT, FOREIGN KEY (category_id) REFERENCES expense_categories(id))''')
        
        c.execute('''CREATE TABLE IF NOT EXISTS incomes 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, amount REAL, 
                     currency TEXT DEFAULT 'VND', source TEXT, income_date TEXT, note TEXT, 
                     created_at TEXT)''')
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS group_admins 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT, group_id INTEGER, admin_id INTEGER, 
                     granted_by INTEGER, can_view INTEGER DEFAULT 0, can_edit INTEGER DEFAULT 0, 
                     can_delete INTEGER DEFAULT 0, can_manage INTEGER DEFAULT 0, created_at TEXT, 
                     UNIQUE(group_id, admin_id))''')
        
        c.execute('''CREATE TABLE IF NOT EXISTS permission_logs 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT, group_id INTEGER, action_by INTEGER, 
                     target_user INTEGER, action TEXT, old_role TEXT, new_role TEXT, created_at TEXT)''')
        
        c.execute('''CREATE TABLE IF NOT EXISTS group_owners 
                    (group_id INTEGER PRIMARY KEY, owner_id INTEGER, created_at TEXT)''')
        
        c.execute('''CREATE TABLE IF NOT EXISTS permissions 
                    (id INTEGER PRIMARY KEY AUTOINCREMENT, group_id INTEGER, user_id INTEGER, 
                     granted_by INTEGER, is_approved INTEGER DEFAULT 1, role TEXT DEFAULT 'user', 
                     can_view_all INTEGER DEFAULT 0, can_edit_all INTEGER DEFAULT 0, 
                     can_delete_all INTEGER DEFAULT 0, can_manage_perms INTEGER DEFAULT 0, 
                     created_at TEXT, approved_at TEXT, UNIQUE(group_id, user_id))''')
...
def set_group_owner(group_id, owner_id):
    try:
        ...
        c.execute('''INSERT OR REPLACE INTO group_owners (group_id, owner_id, created_at) VALUES (?, ?, ?)''', 
...
def check_permission(group_id, user_id, permission_type='view'):
    ...
    try:
        ...
        if permission_type == 'view':
            ...
            return can_edit == 1 or can_manage == 1
        elif permission_type == 'delete':
            return can_delete == 1 or can_manage == 1
...
def add_alert(user_id, symbol, target_price, condition):
    ...
    try:
        ...
        c.execute('''INSERT INTO alerts (user_id, symbol, target_price, condition, created_at) 
                    VALUES (?, ?, ?, ?, ?)''',
...
def get_user_alerts(user_id):
    ...
    try:
        ...
        c.execute('''SELECT id, symbol, target_price, condition, created_at 
                    FROM alerts WHERE user_id = ? AND is_active = 1 ORDER BY created_at''', (user_id,))
        return c.fetchall()
    except Exception as e:
...
def add_expense_category(user_id, name, budget=0):
    ...
    try:
        ...
        c.execute('''INSERT INTO expense_categories (user_id, name, budget, created_at) 
                    VALUES (?, ?, ?, ?)''',
...
def get_expense_categories(owner_id):
    ...
    try:
        ...
        c.execute('''SELECT id, name, budget, created_at 
                    FROM expense_categories WHERE user_id = ? ORDER BY name''', (owner_id,))
        return c.fetchall()
    except Exception as e:
...
def add_income(owner_id, amount, source, currency='VND', note=""):
    ...
    try:
        ...
        c.execute('''INSERT INTO incomes (user_id, amount, source, income_date, note, created_at, currency) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)''',
...
def add_expense(owner_id, category_id, amount, currency='VND', note=""):
    ...
    try:
        ...
        c.execute('''INSERT INTO expenses (user_id, category_id, amount, note, expense_date, created_at, currency) 
                    VALUES (?, ?, ?, ?, ?, ?, ?)''',
...
def get_recent_incomes(user_id, limit=10):
    ...
    try:
        ...
        c.execute('''SELECT id, amount, source, note, income_date, currency 
                    ...
                    ORDER BY income_date DESC, created_at DESC LIMIT ?''', (user_id, limit))
        return c.fetchall()
    except Exception as e:
...
def get_recent_expenses(user_id, limit=10):
    ...
    try:
        ...
        c.execute('''SELECT e.id, ec.name, e.amount, e.note, e.expense_date, e.currency 
                    ...
                    WHERE e.user_id = ? 
                    ORDER BY e.expense_date DESC, e.created_at DESC LIMIT ?''', (user_id, limit))
        return c.fetchall()
    except Exception as e:
...
def generate_master_report(user_id, password=None):
    try:
        ...
        writer.writerow(['Định dạng:', 'CSV MASTER'])
...
def generate_expense_master_report(user_id, password=None):
    try:
        ...
        incomes = get_recent_incomes(user_id, 5000)
...
def get_expense_menu_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 GẦN ĐÂY", callback_data="expense_recent"),
         InlineKeyboardButton("🔐 Xuất báo cáo", callback_data="export_expense_menu")],
...
async def export_secure_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for tx in transactions:
            writer.writerow([
                tx[0], tx[1], f"{tx[2]:.8f}", f"${tx[3]:,.2f}", tx[4], f"${tx[5]:,.2f}"
            ])
...
async def remove_admin_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if revoke_permission(chat_id, admin_id):
...
async def owner_panel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "addstaff" and len(ctx.args) >= 2:
        ...
        c.execute('''SELECT user_id, username, first_name, created_at 
                    FROM permissions WHERE group_id = ? AND is_approved = 0 AND role = 'user' 
...
async def handle_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "edit_transactions":
            ...
            transactions = get_transaction_detail(target_id)
        ...
        if data == "back_to_main":
            ...
            await query.message.reply_text("👇 Chọn chức năng:", reply_markup=get_main_keyboard(current_user_id))
            return
        
        if data == "back_to_invest":
            msg = f"💰 *MENU ĐẦU TƯ COIN*\n━━━━━━━━━━━━━━━━\n\n🕐 {format_vn_time()}"
            await safe_edit_message(query, msg, reply_markup=get_invest_menu_keyboard(current_user_id, chat_id, chat_type))
            return
        ...
        if data == "show_portfolio":
            ...
            portfolio_data = get_portfolio(target_id)
            ...
            display_name = user_info[0] if user_info and user_info[0] else (user_info[1] if user_info else f"User {target_id}")
        ...
        if data == "show_stats":
            ...
            stats = get_portfolio_stats(target_id)
        ...
        # Group 10: Settings Menu
            ...
            current_display = "🇻🇳 Tiếng Việt" if current_lang == 'VI' else "🇨🇳 中文"
...
async def setup_webhook():
    try:
        ...
        await app.bot.set_webhook(url=webhook_url, allowed_updates=['message', 'callback_query'], 
                                  drop_pending_updates=True, max_connections=render_config.get_worker_count())
...
def smart_startup():
    ...
    def fix_database_constraints():
        ...
        try:
            ...
            if orphan_count > 0:
                logger.warning(f"⚠️ Phát hiện {orphan_count} chi tiêu orphan")
                c.execute('''
    ...
    if render_config.is_render:
        ...
        if os.path.exists('/data'):
            ...
            total_space = stat.f_frsize * stat.f_blocks / (1024 * 1024 * 1024)
